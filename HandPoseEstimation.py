...
class PoseEstiamtion:
    ...
    def printfinger(self,Landmark):
        #print(self.isThumbOpen(Landmark), self.isFirstFingerOpen(Landmark), self.isSecondFingerOpen(Landmark), self.isThirdFingerOpen(Landmark), self.isFourthFingerOpen(Landmark))
        fingers = []
        tipIds = [8,12,16,20]

        #thumb
        if Landmark[4][1] < Landmark[4 - 1][1]:
            fingers.append(1)
        else:
            fingers.append(0)
